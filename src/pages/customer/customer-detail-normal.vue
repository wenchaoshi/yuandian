  ...
  <div id="page" @click="cancle">
    <div class="view customerDetail" >
      <div class="customerDetail-scrollView">
          ...
          <!-- 产品查看次数统计 -->
          <div class="product-cuont flex">
              <div @click="navigator('/add-evolve')"><img src="../../images/add-icon.png" alt="">新建跟进</div>
              <div @click="navigator('/evolve-detail')"><img src="../../images/check-icon.png" alt="">查看跟进</div>
          </div>
          ...
          <div class="content">
            <div>
              <label for="phone"><span>手机号：</span><input id="phone" readonly type="text" :value="customerDetail.customer.wx_phone" placeholder="请输入手机号"></label>
            </div>
            <div>
              <label for="misc-desc"><span>个人描述：</span><input type="text" id="misc-desc" readonly :value="customerDetail.customer.remarks" placeholder="请输入个人描述"></label>
            </div>
            <div class="align-center">
              <button @click.stop='navigator("/customer-information")'>查看完整信息</button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    product_count(){
      //获取查看商品列表的接口
      that.request('/wxapp/roll/message?customer_id='+that.customerId+that.employee_id,{
        success(res){
    ...
    navigator(path) {
      if(path=='/customer-information'){
        ...
        this.$router.push({ path: path, query:{employee_id:this.employee_id,customer_id:this.customerDetail.customer.id} });
      }
    },
    //设置遮罩的class，切换状态
    setClass(target) {
    ...
    get_customerDetail(){
      //获取顾客详情，（预计成交率和实际跟进进度）
      that.request('/wxemployee/customer/detail?customer='+that.customerId+'&customer_id='+that.customerId+that.employee_id,{
        success(res){
          console.log(res)
    ...
    get_tag_list(){
      //获取总标签（店铺标签）
      that.request('/wxemployee/company/tag/list?customer='+this.customerId+'&customer_id='+that.customerId+that.employee_id,{
        success(res){
    ...
    get_now_tag_list(){
      //获取顾客的标签
      that.request('/wxemployee/customer/tag/operate?customer='+this.customerId+'&customer_id='+that.customerId+that.employee_id,{
        success(res){
    ...
    addTag(){
      ...
      that.request('/wxemployee/customer/tag/operate?tag=2006468&customer='+this.customerId+'&customer_id='+that.customerId+that.employee_id,{
        type:'post',
        data:{
          ids:activeTagList
        },
    ...
    setEvolve(e){
      ...
      if (e.target.nodeName.toLowerCase() === 'li') {
        ...
        that.request('/wxemployee/customer/fact/follow?customer='+that.customerId+'&customer_id='+that.customerId+that.employee_id,{
          type:'post',
          data:{
            group_id:index
          },
    ...
    get_talkcount(){
      //获取沟通次数（总和周）
      that.request('/wxapp/customer/talkcount?customer='+that.customerId+'&customer_id='+that.customerId+that.employee_id,{
        success(res){
...
.content 
  ...
  color #999
  border-radius 2px
  box-shadow 0 5px 6px #eee, 0 0 10px #eee
  label 
    display flex
...
.evolve .el-icon-arrow-up:before
  content "\E60C"
  color #fff
  font-size 26px
.view.customerDetail
